...
//
public class LobByteChannelOld implements SeekableByteChannel
{
	...
	private Consumer<LobByteChannelOld> mCloseAction;
	private boolean mWriteMetadata;


	public LobByteChannelOld(BlockAccessor aBlockAccessor, Document aHeader, LobOpenOption aOpenOption) throws IOException
	{
		this(aBlockAccessor, aHeader, aOpenOption, true, DEFAULT_LEAF_SIZE, DEFAULT_COMPRESSOR);
	}


	public LobByteChannelOld(BlockAccessor aBlockAccessor, Document aHeader, LobOpenOption aOpenOption, boolean aWriteMetadata, int aLeafBlockSize, int aCompressorLevel) throws IOException
	{
		if (aHeader == null)
		{
...
//				if (LOG) Console.println("     +-- " + new BlockPointer().unmarshal(mIndirectBlock.read(new byte[SIZE])));
	...
	public LobByteChannelOld setCloseAction(Consumer<LobByteChannelOld> aCloseAction)
	{
...
//					Console.println("\tWrite chunk %d, %d bytes", mChunkIndex, len);
				...
				if (mChunkIndex * SIZE < mIndirectBlock.capacity() && mIndirectBlock.position(mChunkIndex * SIZE).peekInt8() != BlockType.HOLE && mIndirectBlock.position(mChunkIndex * SIZE).peekInt8() != BlockType.HOLE)
				{
...
//					if (LOG) Console.println(bp + " - FREE");
				...
				{
					bp.setBlockType(BlockType.HOLE);
				}
...
//					if (LOG) Console.println(bp + " - READ");
	...
	public LobByteChannelOld readAllBytes(OutputStream aDst) throws IOException
	{
	...
	public LobByteChannelOld writeAllBytes(byte[] aSrc) throws IOException
	{
	...
	public LobByteChannelOld writeAllBytes(InputStream aSrc) throws IOException
	{
		ByteBuffer buf = ByteBuffer.allocate(1024);
		...
		return new InputStream()
		{
			...
			public int read() throws IOException
			{
				if (buffer.position() == buffer.limit())
				{
					...
					buffer.position(0);
					LobByteChannelOld.this.read(buffer);
					buffer.flip();
	...
	public OutputStream newOutputStream()
	{
		return new OutputStream()
		{
			...
			public void write(int aByte) throws IOException
			{
				...
				{
					buffer.flip();
					LobByteChannelOld.this.write(buffer);
					buffer.clear();
				}
			}
	...
	public String toString()
	{
		...
		{
			return String.format("{alloc=%d, phys=%d, logic=%d, pointer=%S}", getMetadata().get(METADATA_ALLOCATED_SIZE), getMetadata().get(METADATA_PHYSICAL_SIZE), mHeader.get(IX_LOGICAL_SIZE, 0L), new BlockPointer().unmarshal(mHeader.get(IX_POINTER)));
		}
		return String.format("{logic=%d, pointer=%S}", mHeader.get(IX_LOGICAL_SIZE, 0L), new BlockPointer().unmarshal(mHeader.get(IX_POINTER)));
	}
